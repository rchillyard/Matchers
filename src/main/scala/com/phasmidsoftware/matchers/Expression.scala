...
/**
  ...
  *
  * This class represents a temporary implementation of an Expression.
  * It is not intended to be used, but I don't want to throw it away just yet.
  * See com.phasmidsoftware.number.mill.Expression for the final implementation.
  *
  * This trait provides a foundation for symbolic computation
  * by defining core operations that can be applied to expressions,
  * including arithmetic, logical, and trigonometric operations.
  */
private[matchers] trait Expression:
  /**
    * Evaluates and retrieves the integer value of this `Expression`.
    *
    * @return the evaluated integer representation of this `Expression`.
    */
  def value: Int

  /**
    * Combines this `Expression` with another `Expression` using the addition (+) operator.
    *
    * @param other the `Expression` to be added to this `Expression`.
    * @return a new `Expression` representing the sum of this `Expression` and the given `Expression`.
    */
  def +(other: Expression): Expression = DyadicExpression(this, other, "+")

  /**
    * Combines this `Expression` with another `Expression` using the multiplication (*) operator.
    *
    * @param other the `Expression` to multiply with this `Expression`.
    * @return a new `Expression` representing the product of this `Expression` and the given `Expression`.
    */
  def *(other: Expression): Expression = DyadicExpression(this, other, "*")
...
// Implicit conversions to wrap primitives
given Conversion[Int, Expression] = i => Valuable(i)  // or Valuable(i)?
given Conversion[String, Expression] = s => Valuable(s.toInt)
...
/**
  ...
  *
  * A Valuable is a leaf node in the expression structure,
  * encapsulating a numeric value. It directly stores a value of type
  * `com.phasmidsoftware.number.Int`.
  *
  * @constructor Creates a Valuable with the specified numeric value.
  * @param value The numeric value encapsulated by this terminal expression.
  */
case class Valuable(value: Int) extends Expression {
  override def toString: String = value.toString
}

case object 𝛑 extends Expression {
  def value: Int = -1
}
...
object Expression {
  ...
  def apply(x: Int): Expression = Valuable(x)
}